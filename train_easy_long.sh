...
OUT_DIR="${1:-logs/drqn_easy_long}"
EPISODES="${2:-1500}"

echo "[train_easy_long] output -> ${OUT_DIR}"
"${PYTHON_BIN}" drqn_minimal.py \
  --preset stable_train \
  --episodes "${EPISODES}" \
  --curriculum-mode distance \
  --curriculum-end-dist 1765 \
  --curriculum-full-at-episode 1200 \
...
echo "[train_easy_long] done"
echo "[train_easy_long] best: ${OUT_DIR}/drqn_torch_best.pt"
