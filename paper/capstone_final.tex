...
% ─────────────────────────────────────────────
\begin{abstract}
Campus evacuation under dynamic disaster conditions requires policies that adapt to heterogeneous populations and degrading infrastructure. This work presents a three-layer decision-support system combining LLM-based behavioral profiling (Layer 1), zone-level coordination (Layer 2), and a persona-aware Deep Recurrent Q-Network (Layer 3) trained on LLM-calibrated disaster parameters. We evaluate the system on the University of Utah road network across three disaster types (blizzard, earthquake, compound) and four severity levels. The persona-aware DRQN achieves reached rates up to 0.79 under light blizzard and maintains above 0.54 under extreme conditions, substantially outperforming heuristic baselines. A cross-disaster fairness analysis across 20 persona types reveals equity gaps of up to 1.0 under earthquake extreme, with visitors and low-familiarity personas being most vulnerable. These results motivate future fairness-constrained policy training.
\end{abstract}
...
\textbf{Evacuation planning and simulation.} Chen et al.~\cite{chen2018} optimize evacuation routes under hazard scenarios using a hybrid simulation-optimization approach, but assume homogeneous agent populations. Agent-based models such as NetLogo-based crowd simulations capture heterogeneity at the pedestrian level but typically rely on hand-crafted behavioral rules rather than learned policies.

\textbf{Deep reinforcement learning for navigation.} Mnih et al.~\cite{mnih2015} demonstrate that deep Q-networks (DQN) can learn human-level navigation policies from raw state input. Hausknecht and Stone~\cite{hausknecht2015} extend DQN with a recurrent memory layer (DRQN) to handle partial observability, making it applicable to graph navigation where agents cannot observe the full network state. Wang et al.~\cite{wang2016} introduce Dueling DQN, which separately estimates state value and action advantages, improving sample efficiency in sparse-reward settings.

\textbf{LLM-augmented simulation and planning.} Large language models have been applied to generate diverse NPC behaviors in social simulations~\cite{meta2024llama3}. The ReAct framework~\cite{yao2023react} extends LLM reasoning with interleaved action execution, which we adapt for zone-level disaster coordination. To our knowledge, no prior work integrates LLM-generated behavioral profiles and scenario parameters directly into the training distribution of an RL navigation agent.

\textbf{Curriculum learning.} Bengio et al.~\cite{bengio2009curriculum} show that training on progressively harder examples improves generalization. We apply curriculum learning across four severity levels, which empirically stabilizes training and improves performance under extreme conditions compared to direct full-severity training.
...
Table~\ref{tab:baseline_vs_drqn} compares heuristic policies and DRQN under blizzard conditions across severity levels (40 agents, 20 runs each). All policies are evaluated under the same blizzard scenario conditions.

\begin{table}[h]
\centering
\caption{Reached rate comparison: baselines vs.\ DRQN under blizzard (40 agents, 20 runs). ``DRQN (prev.)'' refers to the pre-persona checkpoint (\texttt{drqn\_progressive\_severity\_v3\_extreme}), trained with a 37-dim observation vector lacking persona features; included as an upper-bound reference for the learned-policy family.}
...
Blizzard Light    & 0.058 & 0.173 & 0.845 & \textbf{0.790} \\
Blizzard Moderate & 0.022 & 0.100 & 0.752 & \textbf{0.724} \\
Blizzard Severe   & 0.012 & 0.061 & 0.640 & \textbf{0.678} \\
Blizzard Extreme  & 0.010 & 0.031 & 0.519 & \textbf{0.545} \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:sweep} summarizes DRQN performance across all 12 scenario conditions (40 agents, 20 runs per condition, LLM-calibrated parameters).
...
\midrule
Light    & 0.766 & 0.812 & 0.876 & 0.672 \\
Moderate & 0.712 & 0.738 & 0.776 & 0.606 \\
Severe   & 0.698 & 0.671 & 0.627 & 0.672 \\
Extreme  & 0.522 & 0.579 & 0.558 & 0.606 \\
\bottomrule
\end{tabular}
\end{table}

Under light and moderate conditions, staff achieves the highest reached rate due to high campus familiarity, high compliance, and low panic. However, the role ordering shifts at severe and extreme severity: staff rates decline faster than student rates, likely because the curriculum-trained policy was not exposed to enough staff-specific extreme scenarios. The visitor role shows high reached rates at severe and extreme conditions, but this is an artifact of small sample sizes (1--2 visitor agents per run); statistical confidence is low for these estimates, as noted in the limitations.

At the persona level, the fairness gap at extreme blizzard severity is 0.557 (mobility\_impaired: 0.875 vs.\ research\_scientist: 0.318). The mobility\_impaired value is high here due to small sample sizes (visitor-class personas average 1--2 agents per run, flagged with confidence warnings in the output). The five most vulnerable personas at extreme severity are:

\begin{enumerate}
  \item \textbf{research\_scientist} (faculty): 0.318 -- moderate campus familiarity but elevated observation error and lower compliance under panic
  \item \textbf{conference\_attendee} (visitor): 0.389 -- low familiarity, high observation error
  \item \textbf{part\_time\_student} (student): 0.396 -- lower compliance and reduced familiarity compared to full-time students
  \item \textbf{freshman\_student} (student): 0.428 -- high panic level combined with limited campus experience
  \item \textbf{student\_with\_anxiety} (student): 0.439 -- panic-induced compliance collapse
\end{enumerate}
...
\textbf{Layer 2 integration.} The LLM Zone Coordinator (Layer 2) operates in a ReAct loop over four tools: shelter status, zone population, road conditions, and distance matrix. In offline evaluation (without API key), Layer 2 falls back to algorithmic zone assignment, making Configs B and C identical. The \texttt{eval\_pipeline\_integration.py} script provides the framework to run a direct B-vs-C comparison once an API key is available. Quantifying the marginal contribution of Layer 2 is left as future work.
