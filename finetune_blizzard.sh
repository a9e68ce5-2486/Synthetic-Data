...
OUT_DIR="${2:-logs/drqn_blizzard_finetune}"
EPISODES_STAGE1="${3:-300}"
EPISODES_STAGE2="${4:-300}"

STAGE1_DIR="${OUT_DIR}/moderate_stage"
STAGE2_DIR="${OUT_DIR}/severe_stage"

echo "[finetune_blizzard] init -> ${INIT_CKPT}"
echo "[finetune_blizzard] output -> ${OUT_DIR}"
echo "[finetune_blizzard] stage1 -> moderate (${EPISODES_STAGE1} episodes)"
"${PYTHON_BIN}" drqn_minimal.py \
  --preset stable_train \
  --episodes "${EPISODES_STAGE1}" \
  --curriculum-mode distance \
  --curriculum-start-dist 900 \
...
echo "[finetune_blizzard] stage2 -> severe (${EPISODES_STAGE2} episodes)"
"${PYTHON_BIN}" drqn_minimal.py \
  --preset stable_train \
  --episodes "${EPISODES_STAGE2}" \
  --curriculum-mode distance \
  --curriculum-start-dist 1100 \
  --curriculum-end-dist 2622 \
  --curriculum-freeze-episode -1 \
  --eps-decay 0.998 \
  --eps-end 0.01 \
...
echo "[finetune_blizzard] done"
echo "[finetune_blizzard] moderate best: ${STAGE1_DIR}/drqn_torch_best.pt"
echo "[finetune_blizzard] severe best: ${STAGE2_DIR}/drqn_torch_best.pt"
echo "[finetune_blizzard] linked best: ${OUT_DIR}/drqn_torch_best.pt"
